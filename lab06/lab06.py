...
class Account:
    ...
    def deposit(self, amount):
        ...
        self.transactions.append(Transaction(self.transactionID, before, self.balance))
...
def make_change(amount, coins):
    ...
    if amount == smallest:
        return [smallest]
    returned = make_change(amount-smallest, rest)
    if returned == None:
        return make_change(amount, rest)
    if returned != None:
        return [smallest] + returned
    #at least I tried
    """list_to_return = []
    total = 0
    while True:
        ...
        if coins == {}:
            return None
        if total < amount:
            list_to_return.append(min(coins))
            total += min(coins)
            coins = remove_one(coins, min(coins))"""
